...
TEST(input_protocol_service_test, should_get_commit_and_correct_candidate_when_decode) {
	const auto& protocol = modian::common::service::input_protocol_service::parse_instruction_response(R"({"candidate_info":{"payload":"你"},"type":"C"})");
...
TEST(input_protocol_service_test, should_get_update_and_correct_message_when_decode) {
	const auto& protocol = modian::common::service::input_protocol_service::parse_instruction_response(R"({"candidate_info":{"payload":"dia"},"type":"U"})");
...
TEST(input_protocol_service_test, should_get_none_when_decode_empty_message) {
	const auto& protocol = modian::common::service::input_protocol_service::parse_instruction_response(R"({"candidate_info":{"payload":""},"type":"N"})");
